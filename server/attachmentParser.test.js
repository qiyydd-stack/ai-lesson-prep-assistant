...
test("normalizeImageRecognitionError explains incompatible vision APIs", () => {
  const message = normalizeImageRecognitionError({
    error: {
      message:
  ...
  assert.match(message, /当前模型接口不支持图片识别/);
  assert.match(message, /支持视觉输入/);
});

test("isImageRecognitionSupported rejects MiniMax text model configuration", () => {
  const support = isImageRecognitionSupported({
